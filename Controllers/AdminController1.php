...
// Add pool prize break
	...
	{    
        ...
        $check =  $this->security->validateRequired(['matchid','contestid','cpybfrtim','cpypoolstatus'],$input);          
        if(isset($check['error'])) {
              return $response->withJson($check);
          }
        $matchid    = $input['matchid'];
        ...
        if($id){
        	$data['id']= $id['id'];
            $sql     = "UPDATE cpypoolproc SET matchid=:matchid,contestid=:contestid,cpybfrtim=:cpybfrtim,status=:status,poolid=:poolid  WHERE id=:id AND matchid=:matchid"; 
              $stmt    = $this->db->prepare($sql);
              $res=$stmt->execute($data) ;
              $msg="Copy Pool $smsg successfully.";
        }else{
          $stmt = $this->db->prepare("INSERT INTO cpypoolproc (matchid,contestid,cpybfrtim,poolid,status) VALUES (:matchid,:contestid,:cpybfrtim,:poolid,:status)");
          $res=$stmt->execute($data);
         ...
         try{   
			...
			$imgUrl = $baseurl.'/uploads/notifications/';     	
			    if(!empty($userid)){
			    	...
			    	$projectData=["_id"=>1,"title"=>1,'created'=>1,'img'=>1,'message'=>1,'userid'=>$puserid,'sendAll'=>1];
			    	$res=$collection->aggregate([['$match'=>$matchdata],['$project'=>$projectData],['$skip'=>$offset],['$limit'=>$limit],['$sort'=>$rstSort]])->toArray();
				}else{
					//'id'=>['$toString'=>'$_id']
 			$res=$collection->aggregate([['$match'=>['ntype'=>ADMIN_NOTIFY]],['$project'=>["_id"=>1,"title"=>1,'created'=>1,'img'=>1,'message'=>1,'userid'=>$puserid,'sendAll'=>1]],['$sort'=>$rstSort],['$skip'=>$offset],['$limit'=>$limit]])->toArray();
 		}
			   if(!empty($res)){  
                    $resArr['code']  = 0;
					$resArr['error'] = false;
					$resArr['msg']   = 'Notification List.';	
					$resArr['data']  = ["total"=>($res)?count($res):1,"list"=>$res,'imgUrl'=> $imgUrl];						
			     }else{
                    $resArr['code']  = 1;
					$resArr['error'] = true;
					$resArr['msg']   = 'Record not found.';	
