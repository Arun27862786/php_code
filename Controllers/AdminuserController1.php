...
class AdminuserController
{
	...
	public function menusFunc($request, $response)
	{
        ...
        }else if($loginuser['userdata']->usertype == SUBADMIN)
        {        
...
// List Resoures
    ...
    public function updateKycStatusFunc($request,$response)
	{
        ...
        try{        
            ...
            if($updateusertb->execute($params)){ 
					  ...
					  }else if(in_array($atype, ["bank","bankreject"])){
    ...
    public function sendMail($user){  
     ...
     public function deleteNotifyKYC($checkuser,$atype=NUll){
    	...
    	if($checkuser){
    		...
    		else if ($checkuser['ispanverify'] && $checkuser['isbankdverify'] && $checkuser['isemailverify'] && $checkuser['isphoneverify']){
	...
	{    
	    ...
	    if(!in_array($val_type,array_keys($game_type['offer_value_type']))){ 
			 ...
			 return $response->withJson($resArr,$errcode);
	    }else if($pc_ofr_type==$game_type['rewords_type'][REWARDS_POINTS]){
	 ...
	 try{
	 	    ...
	 	    $stmt = $this->db->prepare("UPDATE promocode SET pcode=:pcode,pc_ofr_type=:pc_ofr_type,pc_val=:pc_val,pc_val_type=:pc_val_type,title=:title,updated=:updated,pc_max_val=:pc_max_val,pc_min_val=:pc_min_val,gametype=:gametype,users_limit=:users_limit,sdate=:sdate,edate=:edate,pctype=:pctype WHERE id=:id");
	 	    $data = ["id"=>$id,"pcode"=>strtoupper($pcode),"pc_ofr_type"=>$pc_ofr_type,"pc_val"=>$ofr_value,"pc_val_type"=>$val_type,"title"=>$title,"updated"=>$updated,"pc_max_val"=>$max_val,"pc_min_val"=>$min_val,"gametype"=>$gtype,"users_limit"=>$userslimit,'sdate'=>$sdate,'edate'=>$edate,'pctype'=>$pctype];
	...
	{    
	    ...
	    if(isset($check['error'])) 
	    {
	     ...
	     $max_val 	=  $input['pc_max_val']; 
	     $created   =  time();
	     $status    =  (isset($input['status']))?$input['status']:INACTIVESTATUS;
	    ...
	    if($max_val< $min_val){
	         $resArr = $this->security->errorMessage("Max amount should be greater than min amount");
			 return $response->withJson($resArr,$errcode);
	    }
	 ...
	 try{
	 	    ...
	 	    $stmt = $this->db->prepare("INSERT INTO promocode (pcode,pc_ofr_type,pc_val,pc_val_type,title,status,created,pc_max_val,pc_min_val,gametype,users_limit,sdate,edate,pctype)VALUES(:pcode,:pc_ofr_type,:pc_val,:pc_val_type,:title,:status,:created,:pc_max_val,:pc_min_val,:gametype,:users_limit,:sdate,:edate,:pctype)");

	 	    $data = ["pcode"=>strtoupper($pcode),"pc_ofr_type"=>$pc_ofr_type,"pc_val"=>$ofr_value,"pc_val_type"=>$val_type,"title"=>$title,"status"=>$status,"created"=>$created,"pc_max_val"=>$max_val,"pc_min_val"=>$min_val,"gametype"=>$gtype,"users_limit"=>$userslimit,'sdate'=>$sdate,'edate'=>$edate,'pctype'=>$pctype];
	...
	public function getPromoCode($request, $response){
       ...
       if(!empty($allPCodes)){  
       		...
       		$sqlCount = "SELECT * FROM promocode ".$where_cond." ORDER BY id DESC  ".$paging;
            $stmtCount = $this->db->prepare($sqlCount);
			$stmtCount->execute($params);
			$resCount =  $stmtCount->fetch();
        	$settings = $this->security->getAllDefineKeys();
