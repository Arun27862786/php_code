...
class AdminmatchesController
{
    ...
    // update upcoming Matches day wise
    ...
    // Arun update upcoming Matches
    public function editmastertblmatchupcommingFunc($request, $response)
    {
        global $settings,$baseurl;
        $code    = $settings['settings']['code']['rescode'];
        $errcode = $settings['settings']['code']['errcode'];
        ...
        $input = $request->getParsedBody();
        $check =  $this->security->validateRequired(['team1','team2','series','id'], $input);
        if (isset($check['error'])) {
            return $response->withJson($check);
        }
        ...
        $id     =  $input['id'];
        $name = $team1.' vs '.$team2;
        $name2 = $team1.' V/S '.$team2;
        $t1 = str_word_count($team1);
        $t2 = str_word_count($team2);
       
        $first =strtok($team1, " ");
        $new_team1 = substr($first,0,3);
        
        $new_team1_up = strtoupper($new_team1);
        $new_team1_low = strtolower($new_team1);

        $first2 =strtok($team2, " ");
        $new_team2 = substr($first2,0,3);
        
        $new_team2_up = strtoupper($new_team2);
        $new_team2_low = strtolower($new_team2);
       
        try {
            $this->db->beginTransaction();
            $updatematchmaster = $this->db->prepare("UPDATE matchmaster SET team1=:team1,team2=:team2,name=:name,seriesname=:seriesname WHERE unique_id = :id ");
            $updatematchmaster->bindParam(':team1', $team1);
            $updatematchmaster->bindParam(':team2', $team2);
            $updatematchmaster->bindParam(':name', $name);
            $updatematchmaster->bindParam(':seriesname', $series);
            $updatematchmaster->bindParam(':id', $id);
            $updatematchmaster->execute();

            $sql  = "SELECT team1,team2 FROM matches where matchid=:matchid";
            $stmt = $this->db->prepare($sql);
            $stmt->execute(["matchid"=>$id]);
            $res2 = $stmt->fetch();

            $match_team1 =$res2['team1'];
            $match_team2 =$res2['team2'];
          
                $updatematch = $this->db->prepare("UPDATE matches SET team1=:team1,team2=:team2,matchname=:matchname,seriesname=:seriesname WHERE matchid=:matchid");
                $updatematch->bindParam(':team1', $new_team1_up);
                $updatematch->bindParam(':team2', $new_team2_up);
                $updatematch->bindParam(':matchname', $name2);
                $updatematch->bindParam(':seriesname', $series);
                $updatematch->bindParam(':matchid', $id);
                $updatematch->execute();

                $smt = $this->db->prepare("UPDATE matchmeta SET teamname=:teamname,teamid=:teamid WHERE matchid=:matchid And teamname=:teamname2");
                $smt->bindParam(':teamname', $new_team1_up);
                $smt->bindParam(':teamid', $new_team1_low); 
                $smt->bindParam(':teamname2', $match_team1);
                $smt->bindParam(':matchid', $id);
                $smt->execute();

                $smt2 = $this->db->prepare("UPDATE matchmeta SET teamname=:teamname,teamid=:teamid WHERE matchid=:matchid And teamname=:teamname2");
                $smt2->bindParam(':teamname', $new_team2_up);
                $smt2->bindParam(':teamid', $new_team2_low); 
                $smt2->bindParam(':teamname2', $match_team2);
                $smt2->bindParam(':matchid', $id);
                if ($smt2->execute()) {
                    $this->db->commit();
                    $resArr['code']  = 0;
                    $resArr['error'] = false;
  ...
  ON pm.pid=mt.pid LEFT JOIN playertype pt ON pt.id=mt.playertype WHERE mt.matchid=:matchid AND teamid=:teamid ";
    ...
    public function playingoutFunc($request, $response)
    {
        global $settings,$baseurl;
        $code    = $settings['settings']['code']['rescode'];
        $errcode = $settings['settings']['code']['errcode'];
        ...
        $input = $request->getParsedBody();
        $check =  $this->security->validateRequired(['matchid','playingout'], $input);
        ...
        $matchid     = $input["matchid"];
        $playingout  = $input["playingout"];
        
        try {
            $sqlCount = "SELECT * FROM playingout WHERE matchid=:matchid";
            $stmtCount = $this->db->prepare($sqlCount);
            $stmtCount->execute(['matchid'=>$matchid]);
            $Res   =  $stmtCount->fetch();
            if (!empty($Res)) {
                $data = [
                    'matchid'=>$matchid,
                ...
                $stmt = $this->db->prepare("UPDATE playingout SET playingout=:playingout WHERE matchid=:matchid");
            } else {
                $data   = [
                    'matchid'=>$matchid,
                    'playingout'=>$playingout,
                  ];
                $stmt = $this->db->prepare("INSERT INTO playingout (matchid,playingout) VALUES (:matchid,:playingout)");
            }
            if (!empty($stmt->execute($data))) {
                $resArr['code']  = 0;
                $resArr['error'] = false;
                $resArr['msg']   = 'Playingout Set Successfully.';
            } else {
                $resArr['code']   = 1;
                $resArr['error']  = true;
                $resArr['msg']    = 'Playingout not Set.';
            }
        } catch (\PDOException $e) {
            $resArr['code']  = 1;
            $resArr['error'] = true;
...
//         }
                ...
                if (!empty($players)) {
                    foreach ($players as $player) {
                        $data2 = [
                              ...
                              'credit'=>$player['credit'],
                              'isplaying'=>$player['isplaying'],
                              'isplayingbot'=>$player['isPlayingBot']
                            ];
                
                        $stmt2 = $this->db->prepare("INSERT INTO matchmeta (matchid,pid,teamname,teamid,playerimg,playertype,pts,credit,isplaying,isplayingbot) VALUES (:matchid,:pid,:teamname,:teamid,:playerimg,:playertype,:pts,:credit,:isplaying,:isplayingbot)");
                        $stmt2->execute($data2);
    ...
    public function getmatcheditFunc($request, $response)
    {
        ...
        try {
            ...
            $sql = "SELECT mt.matchid,mt.matchname,mt.team1,mt.team2,CONCAT('".$logoUrl."',mt.team1logo) as team1logourl,mt.team1logo as team1logoname, CONCAT('".$logoUrl."',mt.team2logo) as team2logourl,mt.team2logo as team2logoname,mt.gametype,mt.gameid,mt.seriesid,mt.seriesname,mt.totalpoints,mt.mtype,p.playingout FROM matches mt LEFT JOIN playingout p on mt.matchid=p.matchid WHERE mt.matchid=:matchid" ;
            $stmt = $this->db->prepare($sql);
            $stmt->execute(["matchid"=>$matchid]);
            $matches =  $stmt->fetch();
         
            $pimgUrl = $baseurl.'/uploads/players/';
            $sql2  = "SELECT mt.pid,mt.teamname,mt.teamid,mt.pts,mt.credit,CONCAT('".$pimgUrl."',mt.playerimg) as pimg,mt.playerimg,mt.playertype,mt.isplaying,mt.isplayingbot,pt.name as ptype,pm.pname FROM matchmeta mt LEFT JOIN playertype pt ON pt.id=mt.playertype LEFT JOIN playersmaster pm ON mt.pid=pm.pid WHERE mt.matchid=:matchid" ;
            $stmt2 = $this->db->prepare($sql2);
