...
/*
        ...
        {         	
			...
			$updateusertb->bindParam(':ip',$ip);
	            ...
	            if($updateusertb->execute()){
				   	...
				   	//rummy login
				   	if(!is_null($userInfoRes['passwordplain']) && !empty($userInfoRes['passwordplain']) ){
					   	$passPlainDec = $this->security->my_decrypt($userInfoRes['passwordplain'],\Security::$salt);
					   	if(!empty($passPlainDec)){
						   	$rumregdata=['phone'=>$userInfoRes['phone'],'email'=>$userInfoRes['email'],'username'=>$userInfoRes['phone'],'password'=>$passPlainDec,'sid'=>$token];
    ...
    $otp = \Security::generateOtp();
        try{     
	        ...
	        $stmt = $this->db->prepare("INSERT INTO userstemp (phone,email,otp,refercode,ip,created,password,passwordplain) VALUES (:phone,:email,:otp,:refercode,:ip,:created,:password,:passwordplain)");
			$params = ["phone"=>$phone,"email"=>$email,"otp"=>$otp,"refercode"=>$refercode,"ip"=>$ip,"created"=>$created,"password"=>$password,"passwordplain"=>$passwordplain];			
    ...
    try{     
        ...
        $stmt = $this->db->prepare("SELECT id,phone,email,refercode,password,passwordplain,socialid,logintype FROM userstemp WHERE phone = :phone AND otp = :otp ORDER BY id DESC");
		...
		$stmt = $this->db->prepare("INSERT INTO users (phone,email, password,ip,browser,created,usertype,devicetoken,devicetype,isphoneverify,wltbns,socialid,logintype,logindate,passwordplain) VALUES (:phone,:email, :password,:ip,:browser,:created,:usertype,:devicetoken,:devicetype,:isphoneverify,:wltbns,:socialid,:logintype,:logindate,:passwordplain)");		
            ...
            $token       = $this->security->tokenGenerate($userInfoRes);
				...
				if(!is_null($userInfoRes['passwordplain']) && !empty($userInfoRes['passwordplain'])){
				   	...
				   	if(!empty($passPlainDec)){
					   	...
					   	$rumregdata=['phone'=>$userInfoRes['phone'],'email'=>$userInfoRes['email'],'username'=>$unm,'password'=>$passPlainDec,'sid'=>$token];
					   	$rumRes = $this->security->callCurl("login",$rumregdata,"POST");
					   	$rumRes1 = $this->security->callCurl("triggerAfterLogin",$rumregdata,"POST");
				    }
				}
	...
	if(!in_array($logintype,['F','G']))
    {
   	...
   	if(!empty($ckUsrScl)){
		...
		if($ckUsrScl['status'] == INACTIVESTATUS || $ckUsrScl['status'] == BLOCK )
		{
			...
			//rummy login
			if(!is_null($ud['userinfo']['passwordplain']) && !empty($ud['userinfo']['passwordplain'])){
			   	$passPlainDec = $this->security->my_decrypt($ud['userinfo']['passwordplain'],\Security::$salt);
			   	$unm = $ud['userinfo']['socialid'];
			   	unset($ud['userinfo']['passwordplain']);
			   	unset($ud['userinfo']['socialid']);

			   	if(!empty($passPlainDec)){
				   	$rumregdata=['phone'=>$ud['userinfo']['phone'],'email'=>$ud['userinfo']['email'],'username'=>$unm,'password'=>$passPlainDec,'sid'=>$ud['token']];
				   	$rumRes = $this->security->callCurl("login",$rumregdata,"POST");
				   	$rumRes1 = $this->security->callCurl("triggerAfterLogin",$rumregdata,"POST");
			    }
			}
	...
	public function  paymentRazorpay($request,$response){
		...
		try{
			...
			if($payment && $pdata=$payment->toArray()){
				...
				if(!$transactionchild && $localOrder && $order_id==$pdata['order_id']){
