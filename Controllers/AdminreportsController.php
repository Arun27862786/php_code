...
class AdminreportsController
{
    ...
    public function getwithdrawalReq($request, $response)
    {
        ...
        try {
            ...
            if (!empty($input['search_obj']) && isset($input['search_obj'])) {
                ...
                } elseif ($search_obj['tbl_status']=="1") {
                    $searchSql .= " AND w.status=:status ";
    ...
    public function withdrawCancel($request, $response)
    {
        global $settings;
        $code    = $settings['settings']['code']['rescode'];
        $errcode = $settings['settings']['code']['errcode'];
        $input = $request->getParsedBody();
        $check =  $this->security->validateRequired(['amount','user_id','id'], $input);
        if (isset($check['error'])) {
            return $response->withJson($check);
        }
        ...
        try {
            $this->db->beginTransaction();
            $updt = $this->db->prepare("UPDATE withdrawals SET status=:status WHERE id=:id");
            if ($updt->execute(["status"=>$status,"id"=>$id])) {
                $stmt = $this->db->prepare("INSERT INTO transactions (userid, amount, status, txdate, docid , ttype, atype, wlt, prebal, curbal) VALUES (:userid, :amount, :status, :txdate, :docid, :ttype, :atype, :wlt, :prebal, :curbal)");
                $stmt->bindParam(':userid', $uid);
                $stmt->bindParam(':amount', $amount);
                $stmt->bindParam(':status', $TXSSUCCESS);
                $stmt->bindParam(':txdate', $created);
                $stmt->bindParam(':docid', $id);
                $stmt->bindParam(':ttype', $ttype);
                $stmt->bindParam(':atype', $atype);
                $stmt->bindParam(':wlt', $WIN);
                $stmt->bindParam(':prebal', $wltwin);
                $stmt->bindParam(':curbal', $new_wltwin);
                $stmt->execute();
                
                $stmt = $this->db->prepare("UPDATE users SET wltwin=:wltwin WHERE id=:id");
                $stmt->execute(["wltwin"=>$new_wltwin,"id"=>$uid]);
            
                $this->db->commit();
                $resArr['code']  = 0;
                $resArr['error'] = false;
                $resArr['msg'] = 'Successfull Reverse Amount.';
            } else {
                $resArr = $this->security->errorMessage("Not Reverse Amount");
            }
        } catch (\PDOException $e) {
            $resArr['code']  = 1;
            $resArr['error'] = true;
    ...
    public function dashStaticsCount($request, $response)
    {
        ...
        try {
            ...
            $barQuery2="SELECT year,month,day,month_name,COUNT(*) AS `user_count` FROM(SELECT YEAR(from_unixtime(u.created)) AS `year`,
                        MONTH(from_unixtime(u.created)) AS `month`,
                        DAY(from_unixtime(u.created)) AS `day`,
                        MONTHNAME(from_unixtime(u.created)) AS `month_name`
                        FROM  fancydb.users u where YEAR(from_unixtime(u.created)) = YEAR(CURDATE()) And
                        MONTH(from_unixtime(u.created)) = MONTH(CURDATE())) AS inc GROUP BY `day`;";
            $stmt2 = $this->db->prepare($barQuery2);
            $stmt2->execute();
            $barChatData2 = $stmt2->fetchAll();
            $month_name = $barChatData2[0]['month_name'];

            $date = new \DateTime(date('Y').'-'.$month_name.'-01'); 
            ...
            if (!empty($user)) {
                ...
                'barChartData2'=>($barChatData2)?['labels'=>array_column($barChatData2, 'day'),'data'=>array_column($barChatData2, 'user_count'),'month_name'=>$month_name,'last_day_this_month'=>$last_day_this_month]:[],
                ];
            } else {
                $resArr['code']  = 1;
                $resArr['error'] = true;
    ...
    public function onePageReportFilter($request, $response)
    {
        ...
        try {
            ...
            $stmt->execute();
            $games = $stmt->fetchAll();
            $rtype = ['teamwise'=>'Team Wise','loginhis'=>'Login History','joining'=>'Joining','balancehis'=>'Balance History','transactions'=>'Transactions','profitloss'=>'Profit Loss','universalac'=>'Universal Account',"leaderboard"=>"Leaderboard Downloaded Users","referralusers"=>"Referral Users"];
            if (!empty($rtype)) {
                $resArr['code']  = 0;
                $resArr['error'] = false;
                $resArr['msg']   = 'Results';
    ...
    public function onePageReport($request, $response)
    {
        ...
        try {
            ...
            } elseif ($type == "referralusers") {
                $res=$this->reportReferralUsers($input);
            } else {
                $res = [];
                //$this->report($input);
            }
    ...
    public function reportReferralUsers($input)
    {
        $dataRes 	= 	[];
        $userid  	= 	$input['userid'];
        $fromdate  	= 	$input['fromdate'];
        ...
        if (!empty($userid)) {
            $filter .= " AND ref.referedby=:userid";
            $params = $params + ["userid"=>$userid];
        }
        if (!empty($input['fromdate'])) {
            $fromdate = strtotime($input['fromdate']);
            $filter  .= " AND ref.created >= :fromdate ";
            $params = $params + ["fromdate"=>$fromdate];
        }
        if (!empty($input['todate'])) {
            $todate = strtotime($input['todate']);
            $filter  .= " AND ref.created <= :todate ";
            $params = $params + ["todate"=>$todate];
        }
         
        $sqlCount 	= "SELECT count(ref.id) as total FROM referralby ref INNER JOIN users u ON u.id = ref.userid LEFT JOIN users r ON r.id=ref.referedby  WHERE 1=1 ".$filter;
        $stmtCount 	= $this->db->prepare($sqlCount);
        $stmtCount->execute($params);
        $resCount 	=  $stmtCount->fetch();
        //CONCAT(r.id,r.name,' ',r.email,' ',r.phone) as rtname
        $sql = "SELECT CONCAT(u.email,' (',u.phone,')') as uname ,CONCAT(r.email,' (',r.phone,')') as rtname FROM referralby ref INNER JOIN users u ON u.id = ref.userid  INNER JOIN users r ON r.id=ref.referedby WHERE 1=1".$filter.$paging;
        $stmt = $this->db->prepare($sql);
        //$params  =  ["matchid"=>$matchid,"userid"=>$uid];
        $stmt->execute($params);
        $res =  $stmt->fetchAll();

        $dataRes['heading'] = ['rtname'=>'Referral From','uname'=>'Referral Used'];
    ...
    public function reportTransactionHis($input)
    {
        ...
        $sql = "SELECT u.email,u.phone,t.txid,t.amount,t.status,t.ttype,t.atype,t.wlt,t.txdate,t.curbal  FROM transactions t INNER JOIN users u ON t.userid=u.id WHERE 1=1 ".$filter." ORDER BY t.id DESC ".$paging;
        $stmt = $this->db->prepare($sql);
        $stmt->execute($params);
        $res = $stmt->fetchAll();
        $des  	 = [
        ...
        $dataRes['heading'] = ['email'=>'Email','phone'=>'Phone','txid'=>'Transactions Id','amount'=>'Amount','status'=>'Status','ttype'=>'Transactions Type','curbal'=>'Balance','description'=>'Description','txdate'=>'Transactions Date'];
        $dataRes['total'] 	= $resCount['total'];
        $dataRes['list'] 	= $resDt;
        return $dataRes;
    }
    ...
    public function reportUniversalAccount($input)
    {
        ...
        foreach ($matches as $match) {
            ...
            $sql     = "SELECT IFNULL(SUM(jc.fees),0) as fees FROM joincontests jc Inner Join matchcontestpool mcp ON  (mcp.contestmetaid=jc.poolcontestid AND mcp.matchid=:matchid) WHERE jc.matchid=:matchid AND mcp.iscancel=0";
            $stmt 	 = $this->db->prepare($sql);
        ...
        foreach ($matches as $match) {
            ...
            $sql     = "SELECT IFNULL(SUM(jc.fees),0) as fees, IFNULL(SUM(jc.winbal),0) as winbal FROM joincontests jc Inner Join matchcontestpool mcp ON  (mcp.contestmetaid=jc.poolcontestid AND mcp.matchid=:matchid) WHERE jc.matchid=:matchid AND mcp.iscancel=0";
            $stmt 	 = $this->db->prepare($sql);
            $stmt->execute(['matchid'=>$matchid]);
            $res 	 =  $stmt->fetch();
